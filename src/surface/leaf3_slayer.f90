!##############################################################################
Subroutine stars (ustar,tstar,rstar,ths,rvs,thetacan,can_rvap,zts,patch_rought  &
   ,patch_roughm,vels_pat,dtllohcc,dens,dtll)

use rconstants

implicit none

real :: ustar,tstar,rstar,ths,rvs,thetacan,can_rvap,zts,patch_rought  &
       ,patch_roughm,vels_pat,dtllohcc,dens,dtll
...
! Adele - calculate ustar with zm
a2 = (vonk / log(zts / patch_roughm)) ** 2
c1 = a2 * vels_pat
...
if (ths - thetacan > 0.) then   ! STABLE CASE
   ...
   c2 = b * a2 * sqrt(zts / patch_roughm * (abs(ri)))
   cm = csm * c2
   fm = (1. - 2 * b * ri / (1. + 2 * cm))
endif
ustar = max(ustmin,sqrt(c1 * vels_pat * fm))

! Adele - calculate other stars with zt
a2 = (vonk / log(zts / patch_rought)) ** 2
c1 = a2 * vels_pat
ri = 9.8 * zts * (ths - thetacan)  &
   / (.5 * (ths + thetacan) * vels_pat * vels_pat)
...
if (ths - thetacan > 0.) then   ! STABLE CASE
   ...
   c2 = b * a2 * sqrt(zts / patch_rought * (abs(ri)))
   ch = csh * c2
   fh = (1. - 3 * b * ri / (1. + 3 * ch))
endif
