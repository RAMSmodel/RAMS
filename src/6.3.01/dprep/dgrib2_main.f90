...
IF (IA .lt. 8) THEN
   ...
   WRITE(*,'("         YES soil/snow; Using R / RELH")')
   WRITE(*,'("         Upper air names: UGRD,VGRD,TMP,GP,RH")')
   WRITE(*,'("         SWE,SnowDepth names: SNOD,SNOD")')
   WRITE(*,'("         Soil Moisture,Temp names: VSOILM,TSOIL")')
...
elseif(datatype==5)then
 ...
 var3d(2)='VGRD'
 var3d(3)='TMP'
 var3d(4)='GP'    !Geopotential, not Geopotential Height(m)
 var3d(5)='RH'
 !2D snow water and depth variables here if available
 nvar2d=2
 allocate (var2d(nvar2d))
 var2d(1)='SNOD'         !WEASD estimated from SNOD
 var2d(2)='SNOD'         !Snow depth [m water equivalent](be careful)
 !SOIL moisture variables here if available
 nvarsd=2
 allocate (varsd(nvarsd))
 varsd(1)='VSOILM'         !Volumetric Soil Moisture (m3/m3)(fraction)
 varsd(2)='TSOIL'          !Soil temperature (K)



!******************************************************************************
!ECMWF-ERA5 (lat-lon) (Global or subset) *#NEEDS FIXING - DO NOT USE YET**
...
print*,'Nrec: ',nrec
print*,'Frecs: ',frecs(nrec)

!For some latlon projections, mode is not included in header as in Wgrib1
!But we need to reorient projection using mode=128 as in Wgrib1.
!However in grib2 format the lat-lon projections is output South-to-North
!as we need, so we do not need to flip projection for latitude.
if (projection(1:7)=="lat-lon") mode=128
print*,'Mode: ',mode

!Do the dprep stuff

!First some grid navigation bookeeping
if (projection(1:7)=="lat-lon") then
   inproj=1
...
do j=1,nlev
 do i=1,nvar3d
   a=amiss
   matched = .false.
   CALL getfield (a,var3d(i),nrec,frecs,fields,levs,idates,ifhrs &
...
do i=1,nvarsd
  ...
  do j=startlev,endlev
     a=amiss
     matched = .false.
     CALL getfield (a,varsd(i),nrec,frecs,fields,levs,idates,ifhrs &
...
do i=1,nvar2d
   ...
   CALL getfield (a,var2d(i),nrec,frecs,fields,levs,idates,ifhrs &
                ,j,ndate,nhr,matched)
   if(matched) then
...
!************************************************************************
Subroutine getfield (a,type,nrec,frecs,fields,levs,idates,ifhrs &
                     ,iplev,ndate,nhr,matched)

implicit none

real, dimension(*) :: a,frecs
character*(*) type, fields(*)
integer, dimension(*) :: levs,idates,ifhrs
...
if(projection(1:7)=='lat-lon'.and.mode==128)then
   allocate (b(nx*ny))
   if(llisglobal) then
...
elseif(datatype==5.and.dimtype=='2d'.and.vars==1.and.type=='SNOD')then
   print*,'   convert SNOD(SWE in meters) to kg/m2(SWE in mm)'
   do i=1,nx*ny
...
elseif(datatype==5.and.dimtype=='2d'.and.vars==2.and.type=='SNOD')then
   print*,'   convert SNOD(SWE in meters) to snow depth(m) assuming 10:1'
   do i=1,nx*ny
...
do i=1,nx
  ...
  !Do not flip N/S for grib2 format
  !wgrib2 says: input WE:NS output WE:SN
  do j=1,ny
    jj=j
    vout(i,j)=vin(ii,jj)
  enddo
