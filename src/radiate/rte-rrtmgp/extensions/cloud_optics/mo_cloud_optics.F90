...
module mo_cloud_optics
  ...
  type, extends(ty_optical_props), public :: ty_cloud_optics
    ...
    real(wp) :: radcld_lwr = 0._wp, radcld_upr = 0._wp
    real(wp) :: raddrz_lwr = 0._wp, raddrz_upr = 0._wp
    real(wp) :: radrain_lwr = 0._wp, radrain_upr = 0._wp
    real(wp) :: radscol_lwr = 0._wp, radscol_upr = 0._wp
    real(wp) :: radhcol_lwr = 0._wp, radhcol_upr = 0._wp
    real(wp) :: radplate_lwr = 0._wp, radplate_upr = 0._wp
    real(wp) :: radros_lwr = 0._wp, radros_upr = 0._wp
    real(wp) :: radagg_lwr = 0._wp, radagg_upr = 0._wp
    real(wp) :: radhail_lwr = 0._wp, radhail_upr = 0._wp
    ...
    integer  :: cld_nsteps = 0
    integer  :: drz_nsteps = 0
    integer  :: rain_nsteps = 0
    integer  :: scol_nsteps = 0
    integer  :: hcol_nsteps = 0
    integer  :: plate_nsteps = 0
    integer  :: ros_nsteps = 0
    integer  :: agg_nsteps = 0
    integer  :: hail_nsteps = 0
    integer  :: liq_nsteps = 0
    integer  :: ice_nsteps = 0
    ! How big is each step in the table?
    real(wp) :: cld_step_size = 0._wp
    real(wp) :: drz_step_size = 0._wp
    real(wp) :: rain_step_size = 0._wp
    real(wp) :: scol_step_size = 0._wp
    real(wp) :: hcol_step_size = 0._wp
    real(wp) :: plate_step_size = 0._wp
    real(wp) :: ros_step_size = 0._wp
    real(wp) :: agg_step_size = 0._wp
    real(wp) :: hail_step_size = 0._wp
    real(wp) :: liq_step_size = 0._wp
    real(wp) :: ice_step_size = 0._wp
    !
    ! The tables themselves.
    !
    real(wp), dimension(:,:), allocatable :: lut_extcld, lut_ssacld, lut_asycld ! (nsize_cld, nbnd)
    real(wp), dimension(:,:), allocatable :: lut_extdrz, lut_ssadrz, lut_asydrz ! (nsize_drz, nbnd)
    real(wp), dimension(:,:), allocatable :: lut_extrain, lut_ssarain, lut_asyrain ! (nsize_rain, nbnd)
    real(wp), dimension(:,:), allocatable :: lut_extscol, lut_ssascol, lut_asyscol ! (nsize_scol, nbnd)
    real(wp), dimension(:,:), allocatable :: lut_exthcol, lut_ssahcol, lut_asyhcol ! (nsize_hcol, nbnd)
    real(wp), dimension(:,:), allocatable :: lut_extplate, lut_ssaplate, lut_asyplate ! (nsize_plate, nbnd)
    real(wp), dimension(:,:), allocatable :: lut_extros, lut_ssaros, lut_asyros ! (nsize_ros, nbnd)
    real(wp), dimension(:,:), allocatable :: lut_extagg, lut_ssaagg, lut_asyagg ! (nsize_agg, nbnd)
    real(wp), dimension(:,:), allocatable :: lut_exthail, lut_ssahail, lut_asyhail ! (nsize_hail, nbnd)
    real(wp), dimension(:,:), allocatable :: lut_extliq, lut_ssaliq, lut_asyliq ! (nsize_hail, nbnd)
    real(wp), dimension(:,:), allocatable :: lut_extice, lut_ssaice, lut_asyice ! (nsize_hail, nbnd)
...
contains
  ...
  function load_lut(this, band_lims_wvn, &
                    ...
                    lut_exthail, lut_ssahail, lut_asyhail) result(error_msg)
    ...
    real(wp),                   intent(in   ) :: radcld_lwr, radcld_upr, radcld_fac
    real(wp),                   intent(in   ) :: raddrz_lwr, raddrz_upr, raddrz_fac
    real(wp),                   intent(in   ) :: radrain_lwr, radrain_upr, radrain_fac
    real(wp),                   intent(in   ) :: radscol_lwr, radscol_upr, radscol_fac
    real(wp),                   intent(in   ) :: radhcol_lwr, radhcol_upr, radhcol_fac
    real(wp),                   intent(in   ) :: radplate_lwr, radplate_upr, radplate_fac
    real(wp),                   intent(in   ) :: radros_lwr, radros_upr, radros_fac
    real(wp),                   intent(in   ) :: radagg_lwr, radagg_upr, radagg_fac
    real(wp),                   intent(in   ) :: radhail_lwr, radhail_upr, radhail_fac
    ! LUT coefficients
    ! Extinction, single-scattering albedo, and asymmetry parameter for liquid and ice respectively
    real(wp), dimension(:,:), intent(in)    :: lut_extcld, lut_ssacld, lut_asycld
    real(wp), dimension(:,:), intent(in)    :: lut_extdrz, lut_ssadrz, lut_asydrz
    real(wp), dimension(:,:), intent(in)    :: lut_extrain, lut_ssarain, lut_asyrain
    real(wp), dimension(:,:), intent(in)    :: lut_extscol, lut_ssascol, lut_asyscol
    real(wp), dimension(:,:), intent(in)    :: lut_exthcol, lut_ssahcol, lut_asyhcol
    real(wp), dimension(:,:), intent(in)    :: lut_extplate, lut_ssaplate, lut_asyplate
    real(wp), dimension(:,:), intent(in)    :: lut_extros, lut_ssaros, lut_asyros
    real(wp), dimension(:,:), intent(in)    :: lut_extagg, lut_ssaagg, lut_asyagg
    real(wp), dimension(:,:), intent(in)    :: lut_exthail, lut_ssahail, lut_asyhail
    character(len=128)    :: error_msg
    ! -------
    !
    ! Local variables
    !
    integer :: nbnd, nsize_cld, nsize_drz, nsize_rain, nsize_scol, nsize_hcol, &
    ...
    !
    nsize_cld = size(lut_extcld,dim=1)
    nsize_drz = size(lut_extdrz,dim=1)
    nsize_rain = size(lut_extrain,dim=1)
    nsize_scol = size(lut_extscol,dim=1)
    nsize_hcol = size(lut_exthcol,dim=1)
    nsize_plate = size(lut_extplate,dim=1)
    nsize_ros = size(lut_extros,dim=1)
    nsize_agg = size(lut_extagg,dim=1)
    nsize_hail = size(lut_exthail,dim=1)
    nbnd      = size(lut_extcld,dim=2)
    !
    ! Error checking
...
!    if(error_msg /= "") return
    ...
    this%cld_nsteps = nsize_cld
    this%drz_nsteps = nsize_drz
    this%rain_nsteps = nsize_rain
    this%scol_nsteps = nsize_scol
    this%hcol_nsteps = nsize_hcol
    this%plate_nsteps = nsize_plate
    this%ros_nsteps = nsize_ros
    this%agg_nsteps = nsize_agg
    this%hail_nsteps = nsize_hail
    this%cld_step_size = (radcld_upr - radcld_lwr)/real(nsize_cld-1,wp)
    this%drz_step_size = (raddrz_upr - raddrz_lwr)/real(nsize_drz-1,wp)
    this%rain_step_size = (radrain_upr - radrain_lwr)/real(nsize_rain-1,wp)
    this%scol_step_size = (radscol_upr - radscol_lwr)/real(nsize_scol-1,wp)
    this%hcol_step_size = (radhcol_upr - radhcol_lwr)/real(nsize_hcol-1,wp)
    this%plate_step_size = (radplate_upr - radplate_lwr)/real(nsize_plate-1,wp)
    this%ros_step_size = (radros_upr - radros_lwr)/real(nsize_ros-1,wp)
    this%agg_step_size = (radagg_upr - radagg_lwr)/real(nsize_agg-1,wp)
    this%hail_step_size = (radhail_upr - radhail_lwr)/real(nsize_hail-1,wp)
    ! Allocate LUT coefficients
    allocate(this%lut_extcld(nsize_cld, nbnd), &
             this%lut_ssacld(nsize_cld, nbnd), &
             this%lut_asycld(nsize_cld, nbnd), &
             this%lut_extdrz(nsize_drz, nbnd), &
             this%lut_ssadrz(nsize_drz, nbnd), &
             this%lut_asydrz(nsize_drz, nbnd), &
             this%lut_extrain(nsize_rain, nbnd), &
             this%lut_ssarain(nsize_rain, nbnd), &
             this%lut_asyrain(nsize_rain, nbnd), &
             this%lut_extscol(nsize_scol, nbnd), &
             this%lut_ssascol(nsize_scol, nbnd), &
             this%lut_asyscol(nsize_scol, nbnd), &
             this%lut_exthcol(nsize_hcol, nbnd), &
             this%lut_ssahcol(nsize_hcol, nbnd), &
             this%lut_asyhcol(nsize_hcol, nbnd), &
             this%lut_extplate(nsize_plate, nbnd), &
             this%lut_ssaplate(nsize_plate, nbnd), &
             this%lut_asyplate(nsize_plate, nbnd), &
             this%lut_extros(nsize_ros, nbnd), &
             this%lut_ssaros(nsize_ros, nbnd), &
             this%lut_asyros(nsize_ros, nbnd), &
             this%lut_extagg(nsize_agg, nbnd), &
             this%lut_ssaagg(nsize_agg, nbnd), &
             this%lut_asyagg(nsize_agg, nbnd), &
             this%lut_exthail(nsize_hail, nbnd), &
             this%lut_ssahail(nsize_hail, nbnd), &
             this%lut_asyhail(nsize_hail, nbnd))
    ...
    ! Load LUT constants
    this%radcld_lwr = radcld_lwr
    this%radcld_upr = radcld_upr
    this%raddrz_lwr = raddrz_lwr
    this%raddrz_upr = raddrz_upr
    this%radrain_lwr = radrain_lwr
    this%radrain_upr = radrain_upr
    this%radscol_lwr = radscol_lwr
    this%radscol_upr = radscol_upr
    this%radhcol_lwr = radhcol_lwr
    this%radhcol_upr = radhcol_upr
    this%radplate_lwr = radplate_lwr
    this%radplate_upr = radplate_upr
    this%radros_lwr = radros_lwr
    this%radros_upr = radros_upr
    this%radagg_lwr = radagg_lwr
    this%radagg_upr = radagg_upr
    this%radhail_lwr = radhail_lwr
    this%radhail_upr = radhail_upr

    ! Load LUT coefficients
    !$acc kernels
    !$omp target
    this%lut_extcld = lut_extcld
    this%lut_ssacld = lut_ssacld
    this%lut_asycld = lut_asycld
    this%lut_extdrz = lut_extdrz
    this%lut_ssadrz = lut_ssadrz
    this%lut_asydrz = lut_asydrz
    this%lut_extrain = lut_extrain
    this%lut_ssarain = lut_ssarain
    this%lut_asyrain = lut_asyrain
    this%lut_extscol = lut_extscol
    this%lut_ssascol = lut_ssascol
    this%lut_asyscol = lut_asyscol
    this%lut_exthcol = lut_exthcol
    this%lut_ssahcol = lut_ssahcol
    this%lut_asyhcol = lut_asyhcol
    this%lut_extplate = lut_extplate
    this%lut_ssaplate = lut_ssaplate
    this%lut_asyplate = lut_asyplate
    this%lut_extros = lut_extros
    this%lut_ssaros = lut_ssaros
    this%lut_asyros = lut_asyros
    this%lut_extagg = lut_extagg
    this%lut_ssaagg = lut_ssaagg
    this%lut_asyagg = lut_asyagg
    this%lut_exthail = lut_exthail
    this%lut_ssahail = lut_ssahail
    this%lut_asyhail = lut_asyhail
    !$acc end kernels
    !$omp end target
...
! ------- Definitions -------
  ...
  function cloud_optics(this, &
    ...
    class(ty_cloud_optics), &
              intent(in   ) :: this
    real(wp), intent(in   ) :: clwp  (:,:,:), &   ! cloud water path (g/m2)
                               reff (:,:,:)
    integer,  intent(in   ) :: rcat (:,:,:)
    ...
    character(len=128)      :: error_msg
    ! ------- Local -------
    logical(wl), dimension(size(clwp,1), size(clwp,2)) :: msk, liqmsk, icemsk
    ...
    real(wp),    dimension(size(clwp,1), size(clwp,2), this%get_nband(),size(clwp,3)) :: &
                tau, taussa, taussag
    integer  :: ncol, nlay, nbnd, ncat
    integer  :: nsizereg
    integer  :: icol, ilay, ibnd, icat, ircat
    ...
    error_msg = ''
    if(.not.(allocated(this%lut_extcld) .or. allocated(this%pade_extliq))) then
...
!    end if
    ...
    if(error_msg == "") then
      ...
      !
      if (allocated(this%lut_extcld)) then
        do icat = 1,ncat

          msk(:,:) = .false.
          do ilay = 1, nlay
            do icol = 1, ncol
              msk(icol,ilay) = clwp(icol,ilay,icat) > 0._wp
            end do
          end do

          if (icat == 1) then !cloud
            call compute_all_from_table(ncol, nlay, nbnd, msk, clwp(:,:,icat), reff(:,:,icat),&
                                        this%cld_nsteps,this%cld_step_size,this%radcld_lwr,  &
                                        this%lut_extcld, this%lut_ssacld, this%lut_asycld,   &
                                        ltau, ltaussa, ltaussag)

          elseif (icat == 2) then !rain
            call compute_all_from_table(ncol, nlay, nbnd, msk, clwp(:,:,icat), reff(:,:,icat),&
                                        this%rain_nsteps,this%rain_step_size,this%radrain_lwr,  &
                                        this%lut_extrain, this%lut_ssarain, this%lut_asyrain,   &
                                        ltau, ltaussa, ltaussag)

          elseif (icat == 5) then !aggregates
            call compute_all_from_table(ncol, nlay, nbnd, msk, clwp(:,:,icat), reff(:,:,icat),&
                                        this%agg_nsteps,this%agg_step_size,this%radagg_lwr,  &
                                        this%lut_extagg, this%lut_ssaagg, this%lut_asyagg,   &
                                        ltau, ltaussa, ltaussag)

          elseif (icat == 6 .or. icat == 7) then !graupel or hail
            call compute_all_from_table(ncol, nlay, nbnd, msk, clwp(:,:,icat), reff(:,:,icat),&
                                        this%hail_nsteps,this%hail_step_size,this%radhail_lwr,  &
                                        this%lut_exthail, this%lut_ssahail, this%lut_asyhail,   &
                                        ltau, ltaussa, ltaussag)
 
          elseif (icat == 8) then !drizzle
            call compute_all_from_table(ncol, nlay, nbnd, msk, clwp(:,:,icat), reff(:,:,icat),&
                                        this%drz_nsteps,this%drz_step_size,this%raddrz_lwr,  &
                                        this%lut_extdrz, this%lut_ssadrz, this%lut_asydrz,   &
                                        ltau, ltaussa, ltaussag)

          else !pristine ice or snow
            do ircat = 6,9 !The ice habits
              do ilay = 1, nlay
                do icol = 1, ncol
                  icemsk(icol,ilay) = (msk(icol,ilay) .and. rcat(icol,ilay,icat) == ircat)
                end do
              end do
             
              if (ircat == 6) then
                call compute_all_from_table(ncol, nlay, nbnd, icemsk, clwp(:,:,icat), reff(:,:,icat),&
                                      this%hcol_nsteps,this%hcol_step_size,this%radhcol_lwr,  &
                                      this%lut_exthcol, this%lut_ssahcol, this%lut_asyhcol,   &
                                      itau, itaussa, itaussag)
            
              elseif (ircat == 7) then
                call compute_all_from_table(ncol, nlay, nbnd, icemsk, clwp(:,:,icat), reff(:,:,icat),&
                                      this%scol_nsteps,this%scol_step_size,this%radscol_lwr,  &
                                      this%lut_extscol, this%lut_ssascol, this%lut_asyscol,   &
                                      itau, itaussa, itaussag)

              elseif (ircat == 8) then
                call compute_all_from_table(ncol, nlay, nbnd, icemsk, clwp(:,:,icat), reff(:,:,icat),&
                                      this%plate_nsteps,this%plate_step_size,this%radplate_lwr,  &
                                      this%lut_extplate, this%lut_ssaplate, this%lut_asyplate,   &
                                      itau, itaussa, itaussag)

              elseif (ircat == 9) then
                call compute_all_from_table(ncol, nlay, nbnd, icemsk, clwp(:,:,icat), reff(:,:,icat),&
                                      this%ros_nsteps,this%ros_step_size,this%radros_lwr,  &
                                      this%lut_extros, this%lut_ssaros, this%lut_asyros,   &
                                      itau, itaussa, itaussag)

              endif
              ltau = ltau + itau
              ltaussa = ltaussa + itaussa
              ltaussag = ltaussag + itaussag
            enddo
          endif
          tau(:,:,:,icat) = ltau
          taussa(:,:,:,icat) = ltaussa
          taussag(:,:,:,icat) = ltaussag
        enddo
        !Adele - this isn't terribly efficient, should make this a function
        !
          ...
          do ibnd = 1, nbnd
            do ilay = 1, nlay
              do icol = 1,ncol
                ...
                optical_props%tau(icol,ilay,ibnd) = sum(tau(icol,ilay,ibnd,:)) - sum(taussa(icol,ilay,ibnd,:))
              end do
            end do
          end do
        type is (ty_optical_props_2str)
          ...
          do ibnd = 1, nbnd
            do ilay = 1, nlay
              do icol = 1,ncol
                tau_temp    = sum(tau(icol,ilay,ibnd,:))
                taussa_temp = sum(taussa(icol,ilay,ibnd,:))
                optical_props%g  (icol,ilay,ibnd) = sum(taussag(icol,ilay,ibnd,:)) / &
        ...
        type is (ty_optical_props_nstr)
          ...
          error_msg = "cloud optics: no method to calculate cloud optical properties"
!Pade coefficient method not implemented for RAMS categories
        !
...
!                                  itau, itaussa, itaussag)
  ...
  function get_num_ice_roughness_types(this) result(i)
    ...
    stop 'this function isnt used currently by RAMS'
    !i = 0
