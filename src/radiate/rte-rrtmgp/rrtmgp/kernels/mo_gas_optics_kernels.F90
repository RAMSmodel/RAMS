...
contains
  ...
  subroutine interpolation( &
    ...
    integer,     dimension(ncol,nlay), intent(out) :: tropo
    ...
    do ilay = 1, nlay
      do icol = 1, ncol
        ...
        if (log(play(icol,ilay)) > press_ref_trop_log) then
           tropo(icol,ilay)=1
        else
           tropo(icol,ilay)=0
        endif
    ...
    do iflav = 1, nflav
      ...
      do ilay = 1, nlay
        ...
        itropo = 1*tropo(icol,ilay) + 2*(1-tropo(icol,ilay))
  ...
  subroutine compute_tau_absorption(                &
    ...
    integer,     dimension(  nminorlower),           intent(in) :: minor_scales_with_density_lower
      !! generic treatment of minor gases - scales with density (e.g. continuum, collision-induced absorption)?
    integer,     dimension(  nminorupper),           intent(in) :: minor_scales_with_density_upper
    integer,     dimension(  nminorlower),           intent(in) :: scale_by_complement_lower
      !! generic treatment of minor gases - scale by density (e.g. self-continuum) or complement?  
    integer,     dimension(  nminorupper),           intent(in) :: scale_by_complement_upper
    ...
    integer,     dimension(ncol,nlay),               intent(in) :: tropo
    ...
    if(top_at_1) then
      itropo_lower(:, 1) = minloc(play, dim=2, mask=(tropo==1))
      itropo_lower(:, 2) = nlay
      itropo_upper(:, 1) = 1
      !itropo_upper(:, 2) = maxloc(play, dim=2, mask=(.not. tropo))
      itropo_upper(:, 2) = maxloc(play, dim=2, mask=(tropo==0))
    else
      itropo_lower(:, 1) = 1
      itropo_lower(:, 2) = minloc(play, dim=2, mask=(tropo==1))
      itropo_upper(:, 1) = maxloc(play, dim=2, mask=(tropo==0))
      itropo_upper(:, 2) = nlay
    end if
    ! ---------------------
    ! Major Species
  ...
  subroutine gas_optical_depths_minor(ncol,nlay,ngpt,        &
    ...
    integer,     dimension(  nminor),            intent(in   ) :: minor_scales_with_density
    integer,     dimension(  nminor),            intent(in   ) :: scale_by_complement
        ...
        do icol = 1, ncol
          ...
          if(layer_limits(icol,1) > 0) then
            do ilay = layer_limits(icol,1), layer_limits(icol,2)
              ...
              !
              if (minor_scales_with_density(imnr)==1) then
                !
                ...
                if(idx_minor_scaling(imnr) > 0) then  ! there is a second gas that affects this gas's absorption
                  ...
                  if (scale_by_complement(imnr)==1) then ! scale by densities of all gases but the special one
