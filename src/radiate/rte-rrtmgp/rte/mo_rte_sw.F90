...
contains
  ...
  function rte_sw_mu0_full(atmos, top_at_1, &
    ...
    integer     :: icol, ilev
    integer     :: has_dif_bc, do_broadband
    ...
    !$acc        data copyin(inc_flux_dif) if (has_dif_bc==1)
    !$omp target data map(to:inc_flux_dif) if (has_dif_bc==1)
    ...
    !$acc        data copyin(   flux_up_loc,flux_dn_loc,flux_dir_loc) if (   do_broadband==1)
    !$omp target data map(to:   flux_up_loc,flux_dn_loc,flux_dir_loc) if (   do_broadband==1)
    !$acc        data create(   flux_up_loc,flux_dn_loc,flux_dir_loc) if (   do_broadband==0)
    !$omp target data map(alloc:flux_up_loc,flux_dn_loc,flux_dir_loc) if (   do_broadband==0)
    ...
    if(len_trim(error_msg) == 0) then
      select type (atmos)
        class is (ty_optical_props_1scl)
          ...
          !
          call sw_solver_noscat(ncol, nlay, ngpt, top_at_1, &
        ...
        class is (ty_optical_props_2str)
          ...
          !
          call sw_solver_2stream(ncol, nlay, ngpt, top_at_1, &
