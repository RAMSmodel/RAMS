...
contains
  ...
  subroutine lw_solver_noscat_oneangle(ncol, nlay, ngpt, top_at_1, D, weight,                              &
    ...
    integer,                               intent(in   ) :: do_rescaling
  ...
  subroutine lw_solver_noscat(ncol, nlay, ngpt, top_at_1, &
    ...
    if(nmus > 1) then
      if(   do_broadband==1) deallocate(this_broadband_up, this_broadband_dn)
      if(   do_broadband==0) deallocate(this_flux_up,        this_flux_dn)
      if(   do_Jacobians==1) deallocate(this_flux_upJac)
    end if
  ...
  subroutine sw_solver_2stream (ncol, nlay, ngpt, top_at_1,  &
    ...
    ! -------------------------------------------
    real(wp), dimension(ncol,nlay     ) :: broadband_sup, broadband_sdn, broadband_sdir
    ...
    if(do_broadband==1) then
      ...
      call zero_array(ncol, nlay, broadband_sup)
      call zero_array(ncol, nlay, broadband_sdn)
    end if
    ...
    do igpt = 1, ngpt
      ...
      if(do_broadband==1) then
        ...
        broadband_sup (:,:) = broadband_sup (:,:) + source_up (:,:)
        broadband_sdn (:,:) = broadband_sdn (:,:) + source_dn (:,:) 
      else
