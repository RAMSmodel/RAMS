...
contains
  ...
  function rte_lw(optical_props, top_at_1, &
    ...
    integer      :: n_quad_angs
    integer      :: using_2stream, do_Jacobians, do_broadband
    ...
    if (do_Jacobians==1 .and. check_extents) then
    ...
    !
    using_2stream = 0! .false.
    if(present(use_2stream)) then
       if(use_2stream) using_2stream=1
    endif
      ...
      select type (optical_props)
        class is (ty_optical_props_1scl)
          ...
          call lw_solver_noscat(ncol, nlay, ngpt,                 &
                                ...
                                do_Jacobians, sources%sfc_source_Jac, jacobian, &
                                0,  optical_props%tau, optical_props%tau)
                                                      ! The last two arguments won't be used since the
                                                      ! third-to-last is .false. but need valid addresses
          !$acc        end data
          !$omp end target data
        class is (ty_optical_props_2str)
          if (using_2stream==1) then
            !
            ! two-stream calculation with scattering
            !
            call lw_solver_2stream(ncol, nlay, ngpt, top_at_1, &
            ...
            call lw_solver_noscat(ncol, nlay, ngpt,                 &
                                  ...
                                  do_broadband, flux_up_loc, flux_dn_loc,     &
                                  do_Jacobians, sources%sfc_source_Jac, jacobian, &
                                  1,  optical_props%ssa, optical_props%g)
            !$acc        end data
            !$omp end target data
          endif
