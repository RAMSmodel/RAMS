...
!     ----------------
      ...
      ncwl,  &     !Cloud water mixing ratio                  (kg/kg).
...
! LOCAL VARIABLES:
      ...
      real (kind=dbl_kind), dimension(len,nlm):: &
       ...
       ncwl_loc, &  !Local cloud water mixing ratio              (kg/kg).
       ...
       nwrho,  &    !Density of cloud water                     (g/m^-3).
      ...
      real (kind=dbl_kind), dimension(:,:), allocatable:: &
       ...
       nwrho_day, & !As cwrho,but for daytime grid-points  (g/m^-3).
      ...
      real (kind=dbl_kind), dimension(:), allocatable::  &    
       radvbc_day, & !As radvbc_loc, but for daytime grid-pts (W/m^-2).
       radvdc_day, & !As radvbc_loc, but for daytime grid-pts (W/m^-2).
       radnbc_day, & !As radnbc_loc, but for daytime grid-pts (W/m^-2).
       radndc_day, & !As radnbc_loc, but for daytime grid-pts (W/m^-2).
       radvbcc_day, & !As radvbcc, but for daytime grid-pts    (W/m^-2).
       radvdcc_day, & !As radvbcc, but for daytime grid-pts    (W/m^-2).
       radnbcc_day, & !As radnbc_loc, but for daytime grid-pts (W/m^-2).
       radndcc_day !As radnbc_loc, but for daytime grid-pts (W/m^-2).
...
!      enddo
      ...
      do l = 1, nnp
         ...
         nwrho(1:len,l) = den*ncwl_loc(1:len,l)*acld_loc(1:len,l)
...
!     fluxes are made for daytime grid-points only: 
      ...
      nwrho_day(1:nday,1:nnp) = nwrho(iday(1:nday),1:nnp)
