...
int main(int argc, char **argv) {
	...
	if (output_type == TEXT) open_parm[1] = '\0';
        ...
        //Saleeby:adding this if statement instead of the 4 commented lines below
        ...
        //Saleeby:commented these 4 lines
	//if ((dump_file = fopen(dump_file_name,open_parm)) == NULL) {
	//    fprintf(stderr,"could not open dump file\n");
	//    exit(8);
        //}
...
#endif
        //Saleeby: adding this 1 line below
        if (verbose <= 0 && dump_file != stdout) {
        //Saleeby: commenting out 1 line below. Replace with 1 line above.
        //if (verbose <= 0) {
...
static const struct ParmTable *Parm_Table(unsigned char *pds) {
    ...
    if ((ptable > 3 || (PDS_PARAM(pds)) > 127) && missing_count++ == 0) {
        //Saleeby: commenting out this print statement below
	/*fprintf(stderr,
